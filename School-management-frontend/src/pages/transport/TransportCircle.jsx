...
import {
  ...
  MoreVertical, List, CircleDot, Edit3, Trash2, Eye,
  Check, X, RotateCcw, ChevronRight, RefreshCw, AlertCircle
} from 'lucide-react';

export default function TransportCircle({ initialView = 'list' }) {
  ...
  const fetchCircles = async () => {
    try {
      setLoading(true);
      const res = await API.get('/transport/circles');
      setCircles(Array.isArray(res.data) ? res.data : []);
    } catch (err) {
      console.error('Error fetching circles:', err);
      showToast('Failed to load transport circles');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSave = async (e) => {
    ...
    if (!formData.name.trim()) {
      showToast('Please enter Circle Name');
      return;
    }
    ...
    try {
      ...
      if (viewMode === 'edit' && currentEditId) {
        await API.put(`/transport/circles/${currentEditId}`, formData);
        showToast('Transport Circle updated successfully!');
      } else {
        await API.post('/transport/circles', formData);
        showToast('Transport Circle created successfully!');
      }
      ...
      showToast(err.response?.data?.message || 'Error saving transport circle');
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleDelete = async (id) => {
    if (!window.confirm('Are you sure you want to delete this Transport Circle?')) return;
    try {
      await API.delete(`/transport/circles/${id}`);
      showToast('Transport Circle deleted successfully');
      setCircles(prev => prev.filter(c => c._id !== id));
    } catch (err) {
      console.error('Error deleting circle:', err);
      showToast('Failed to delete transport circle');
    }
  };
  ...
  const totalPages = Math.ceil(filteredCircles.length / itemsPerPage) || 1;
    ...
    <div className="min-h-screen bg-[#f1f5f9] dark:bg-slate-900 text-slate-800 dark:text-slate-100 flex flex-col justify-between">
      ...
      <div className="p-4 md:p-6 max-w-7xl mx-auto w-full">
        ...
        <div className="flex items-center gap-2 text-xs font-medium text-slate-500 dark:text-slate-400 mb-2">
          ...
          <span>Transport</span>
          <ChevronRight className="w-3 h-3 text-slate-400" />
          <span>Transport Circle</span>
          {viewMode !== 'list' && (
            <>
              <ChevronRight className="w-3 h-3 text-slate-400" />
              <span className="text-slate-700 dark:text-slate-200 font-semibold">
                {viewMode === 'add' ? 'Add Transport Circle' : 'Edit Transport Circle'}
              </span>
            </>
          )}
        </div>
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-4">
          <h1 className="text-xl md:text-2xl font-bold text-slate-900 dark:text-white tracking-tight">
            {viewMode === 'list'
              ? 'Transport Circle'
              : viewMode === 'add'
              ? 'Add Transport Circle'
              : 'Edit Transport Circle'}
          </h1>
          ...
          {viewMode === 'list' ? (
            ...
            >
              ...
              <span>Add Transport Circle</span>
            </button>
          ) : (
            <button
              onClick={() => {
            ...
            >
              ...
              <span>List all Transport Circle</span>
            </button>
          )}
        </div>

        {/* FORM VIEW (Matches Screenshot 3) */}
        {(viewMode === 'add' || viewMode === 'edit') && (
          <div className="bg-white dark:bg-slate-800 rounded-lg shadow-xs border border-slate-200 dark:border-slate-700 p-6 md:p-8">
            <form onSubmit={handleSave} className="space-y-6">
              ...
              {/* Bottom Actions Bar (Exact Screenshot 3 styling) */}
          ...
          <div className="bg-white dark:bg-slate-800 rounded-lg shadow-xs border border-slate-200 dark:border-slate-700 overflow-hidden">
            ...
            <div className="p-3 border-b border-slate-100 dark:border-slate-700 flex items-center justify-between gap-4">
              ...
              <div className="text-xs text-slate-500">
                Total Circles: <span className="font-semibold text-slate-800 dark:text-white">{filteredCircles.length}</span>
              </div>
            </div>
              ...
              <table className="w-full text-left text-xs">
                ...
                <tbody className="divide-y divide-slate-100 dark:divide-slate-700/60">
                  {loading ? (
                    ...
                    <tr>
                      <td colSpan="4" className="py-12 text-center text-slate-400">
                        No transport circles found. Click "Add Transport Circle" above to create one.
                      </td>
                    </tr>
                  ) : (
                    paginatedCircles.map((circle) => (
                      ...
                      >
                        ...
                        <td className="py-3 px-4 text-slate-500 dark:text-slate-400 whitespace-nowrap">
                          {new Date(circle.createdAt).toLocaleString('en-US', {
                            month: 'short',
                            day: 'numeric',
                            year: 'numeric'
                          })}
            ...
            <div className="p-3.5 border-t border-slate-100 dark:border-slate-700 flex flex-col sm:flex-row sm:items-center justify-between gap-4 text-xs text-slate-500">
              <div>
                Showing {filteredCircles.length === 0 ? 0 : (currentPage - 1) * itemsPerPage + 1} to{' '}
                {Math.min(currentPage * itemsPerPage, filteredCircles.length)} of {filteredCircles.length} results
              </div>
