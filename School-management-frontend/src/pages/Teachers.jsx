...
export default function Teachers() {
  ...
  const filteredTeachers = safeTeachers.filter(t => {
    ...
    const empId = t?.employeeId ? String(t.employeeId).toLowerCase() : '';
    const subject = t?.subject ? String(t.subject).toLowerCase() : '';
        ...
        <table className="min-w-full divide-y divide-gray-200 dark:divide-slate-700">
          ...
          <tbody className="bg-white dark:bg-slate-800 divide-y divide-gray-100 dark:divide-slate-700 transition-colors duration-300">
            {currentItems.length === 0 ? (
              ...
              return (
                <tr key={teacher?._id || idx} className="hover:bg-teal-50/30 dark:hover:bg-slate-700/50 transition">
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="flex items-center">
                      ...
                      <div>
                        <div className="text-sm font-bold text-gray-800 dark:text-white">{teacherName}</div>
                      </div>
                    </div>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="text-sm font-medium text-gray-800 dark:text-white">{teacher?.employeeId || 'N/A'}</div>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="text-xs text-gray-500 dark:text-slate-400 bg-gray-100 dark:bg-slate-700 inline-block px-2 py-0.5 rounded-md">{teacher?.subject || 'N/A'}</div>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="text-sm text-gray-700 dark:text-slate-300">{teacher?.contact || 'N/A'}</div>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                    <Link to={`${routePrefix}/teachers/${teacher?._id}`} className="text-teal-600 dark:text-teal-400 hover:text-teal-900 dark:hover:text-teal-300 mr-3 p-1.5 hover:bg-teal-50 dark:hover:bg-teal-900/30 rounded-md inline-block transition-colors duration-300"><Eye className="w-4 h-4"/></Link>
                    {(user?.role === 'admin' || user?.role === 'super-admin') && (
                      <>
                        <Link to={`${routePrefix}/teachers/edit/${teacher?._id}`} className="text-blue-600 dark:text-blue-400 hover:text-blue-900 dark:hover:text-blue-300 mr-3 p-1.5 hover:bg-blue-50 dark:hover:bg-blue-900/30 rounded-md inline-block transition-colors duration-300"><Edit className="w-4 h-4"/></Link>
